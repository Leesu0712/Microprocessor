...
void eval(char *cmdline) 
{
	...
	if (!bg) {
	    addjob(jobs, pid, FG, cmdline);  /* Add the Foreground child to the job list */
	    sigprocmask(SIG_SETMASK, &prev, NULL);  /* Unblock SIGCHLD */
	...
	/* For the Background job */
	    ...
	    addjob(jobs, pid, BG, cmdline);  /* Add the Background child to the job list */
	    sigprocmask(SIG_SETMASK, &prev, NULL);  /* Unblock SIGCHLD */
...
void do_bgfg(char **argv) 
{
    ...
    /* The <job> argument doesn't exist */
    if(argv[1] == NULL) {
	...
	/* The <job> argument is neither JID nor PID */
        else {
    ...
    if(!strcmp(argv[0], "bg")) {
	if(isjid) {  /* Using JID */
	   kill(-(job->pid), SIGCONT);  /* Send a SIGCONT to every process in the process group */
	}
	else {  /* Using PID */
	   kill(-pid, SIGCONT);  /* Send a SIGCONT to every process in the process group */
...
void waitfg(pid_t pid)
{
    ...
    /* Wait for the job state to be changed */
    while ((job->state == FG) && (job)) {
	sleep(1);
    }
    return;
}
...
void sigchld_handler(int sig) 
{
	...
	/* The child is currently stopped */
